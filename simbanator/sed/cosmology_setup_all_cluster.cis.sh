...
if [ -n "$snapshot_prefix" ]; then
    ...
    snapshot_name_expr_region="f'${snapshot_prefix}_snap${snapnum_str}_gal${galaxy_num_str}.h5'"
else
...
then
    ...
    if [ "$subset_type" == "plist" ]; then
        echo "snapshot_name = ${snapshot_name_expr_plist}" >> $filem
    elif [ "$subset_type" == "region" ]; then
        echo "snapshot_name = ${snapshot_name_expr_region}" >> $filem
    else
