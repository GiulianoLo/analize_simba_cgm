...
# BLACK HOLE STUFF
# ===============================================
# almac11 Nenkova i=30 arm: identical to parameters_master.py (dust_on) except
# BH_SED=True (CLUMPY i=30) and the single sightline (see GRID INFORMATION).
...
BH_var = False
nenkova_params = [5, 30, 30, 1.5, 30, 40]  # i=30: near-face-on (type-1-like) CLUMPY view; single fixed-i transmission x Hopkins, injected isotropically
...
# GRID INFORMATION
# ===============================================
# SINGLE sightline for the almac11 Nenkova inclination arms (i90/i60/i30):
# the torus template is injected isotropically (the CLUMPY i only reshapes the
# INPUT spectrum, not the viewing geometry) and the 4-angle test on these
# dust-poor quenched hosts agreed to <0.5%, so extra peel-off angles buy noise
# floor, not information. Compare against sightline THETA=0 (findx=0) of the
# 4-angle dust_on/dust_off arms.
#
...
THETA = [0]   # SINGLE sightline (findx = 0 only) -- ~4x cheaper than the 4-angle set
PHI = [0]
# THETA = [0, 45, 90, 135]   # 4-sightline option (matches the dust_on/dust_off arms)
# PHI   = [0, 90, 180, 270]
